...
FIELDNAMES = ["image_id", "image_w", "image_h", "box_labels", "num_boxes", "boxes", "features"]
...
def parse_args():
    ...
    parser.add_argument(
        "--dataset", dest="dataset", help="training dataset", default="vg", type=str
    )
    parser.add_argument(
        "--net",
        dest="net",
    ...
    parser.add_argument("--split", dest="data_split", help="dataset to use", type=str)
...
def load_image_ids(img_folder: str):
    """ Load a list of (path,image_id tuples). Modify this to suit your data locations. """
    split = []
    folder_path = Path(img_folder)
    for image in folder_path.glob("*.png"):
        split.append([str(image), image.stem])
    return split


def get_detections_from_im(
    fasterRCNN, classes, im_file, image_id, args, conf_thresh=0.2
):
    ...
    (
        ...
        rois_label,
        pooled_feat,
    ) = fasterRCNN(im_data, im_info, gt_boxes, num_boxes, pool_feat=True)
    ...
    if args.cuda > 0:
        keep_boxes = torch.where(
            max_conf >= conf_thresh, max_conf, torch.tensor(0.0).cuda()
        )
    else:
    ...
    for i in range(len(keep_boxes)):
        ...
        box_labels.append(classes[objects[i]+1])
...
def load_model(args):
    ...
    if args.net == "vgg16":
        fasterRCNN = vgg16(
            classes, pretrained=False, class_agnostic=args.class_agnostic
        )
    elif args.net == "res101":
        fasterRCNN = resnet(
            classes, 101, pretrained=False, class_agnostic=args.class_agnostic
        )
    elif args.net == "res50":
        fasterRCNN = resnet(
            classes, 50, pretrained=False, class_agnostic=args.class_agnostic
        )
    elif args.net == "res152":
        fasterRCNN = resnet(
            classes, 152, pretrained=False, class_agnostic=args.class_agnostic
        )
    else:
        ...
        with open(outfile, "a+") as tsvfile:
            ...
            for im_file, image_id in image_ids:
                if int(image_id) in missing:
                    ...
                    if (count % 100) == 0:
                        print(
                            "{:d}/{:d} {:.3f}s (projected finish: {:.2f} hours)".format(
                                count + 1,
                                len(missing),
